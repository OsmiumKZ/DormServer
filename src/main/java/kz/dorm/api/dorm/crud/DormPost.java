...
public class DormPost {
        ...
        if (request.queryParams(DataConfig.DB_DORM_REPORT_UIN) != null &&
            ...
            try (Connection connection = DataBase.getDorm()) {
                if (ControlWrite.isCheckRoom(connection,
                    ...
                    PreparedStatement statement = connection.prepareStatement(StatenentSQL.insert().insertReport());
        ...
        if (request.queryParams(DataConfig.DB_DORM_NAME_F) != null &&
            ...
            try (Connection connection = DataBase.getDorm()) {
                if (ControlWrite.isCheckRoom(connection,
                    ...
                    if (oldRequestId > 0)
                        statement = connection.prepareStatement(StatenentSQL.update().updateRequest());
                    else
                        statement = connection.prepareStatement(StatenentSQL.insert().insertRequest(), Statement.RETURN_GENERATED_KEYS);
    ...
    private static int getOldRequestId(Connection connection, Request request) throws SQLException {
        PreparedStatement statement = connection.prepareStatement(StatenentSQL.select().selectRequestUIN());
    ...
    private static void deleteParent(Connection connection, int idParent) throws SQLException {
        if (idParent > 0) {
            PreparedStatement statement = connection.prepareStatement(StatenentSQL.delete().deleteParentId());
